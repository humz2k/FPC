...
class CParser(PLYParser):
    ...
    def p_postfix_expression_3(self, p):
        """ postfix_expression  : postfix_expression LPAREN argument_expression_list RPAREN
                                ...
                                | postfix_expression LAUNCHL argument_expression_list LAUNCHR LPAREN RPAREN
                                | postfix_expression LAUNCHL argument_expression_list LAUNCHR LPAREN argument_expression_list RPAREN
        """
        if len(p) > 5:
            func_name = c_ast.ID("_LAUNCH_KERNEL_MACRO", self._token_coord(p, 1))
            bounds = p[3]
            assert(len(bounds.exprs) > 1 and len(bounds.exprs) <= 4)
            while (len(bounds.exprs) < 5):
                bounds.exprs.append(c_ast.Constant(type='int',value='0'))
            args = bounds
            if len(p) > 7:
                args.exprs += p[6].exprs
            args.exprs = [p[1]] + args.exprs
            p[0] = c_ast.FuncCall(func_name,args,p[1].coord)
            return
